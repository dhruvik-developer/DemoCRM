...
class HasDynamicPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if role is None:
            logger.warning("Access denied for user %s: No role assigned", getattr(request.user, "user_id", request.user))
            return False

        permission_config = getattr(view, "permission_names", None) or getattr(view, "permission_name", None)

        if permission_config is None:
            logger.warning("Access denied for view %s: No permission configuration defined", view.__class__.__name__)
            return False
        ...
        if not required_permission:
            logger.warning("Access denied for view %s %s: No permission mapped for method", request.method, view.__class__.__name__)
            return False
