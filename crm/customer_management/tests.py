...
class CRMBaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        all_perms = Permission.objects.filter(
            codename__in=[
                ...
                "add_quotation", "change_quotation",
                "view_quotation", "delete_quotation",
        ...
        self.stage1 = CRMService.create_pipeline_stage(
            ...
            name="Stage 1", display_order=1,
        )
        self.stage2 = CRMService.create_pipeline_stage(
            user=self.user, pipeline=self.pipeline,
            name="Stage 2", display_order=2,
        )
        self.stage2_p2 = CRMService.create_pipeline_stage(
            user=self.user, pipeline=self.pipeline2,
            name="Pipeline 2 Stage 1", display_order=1,
        )
        self.inactive_stage = PipelineStage.objects.create(
            pipeline=self.pipeline, name="Inactive Stage",
    ...
    def _create_manager_client(self, permissions=None, username=None, **kwargs):
        ...
        codenames = permissions or [
            "view_lead", "add_lead", "change_lead", "assign_lead",
            "progress_lead", "mark_lead_lost", "reengage_lead", "convert_lead",
            "view_quotation", "add_quotation", "change_quotation",
            "submit_quotation", "approve_quotation", "send_quotation",
            "request_quotation_revision", "accept_quotation",
            "reject_quotation", "delete_quotation", "generate_quotation_pdf",
            "view_customer", "add_customer", "view_activity", "add_activity",
            "view_auditlog", "manage_lead_source", "manage_pipeline",
        ]
        ...
        user = User.objects.create_user(
            ...
            password="Password123!", phone_number=phone,
            role=role,
        )
        client = APIClient()
    ...
    def _create_employee_client(self, permissions=None, username=None, **kwargs):
        ...
        codenames = permissions or [
            ...
            "view_quotation", "add_quotation", "change_quotation",
            "submit_quotation", "send_quotation",
            "request_quotation_revision", "generate_quotation_pdf",
            "view_customer", "add_customer",
            "view_activity", "add_activity",
            "view_leadsource", "view_pipeline", "view_pipelinestage",
        ]
        role.permissions.set(Permission.objects.filter(codename__in=codenames))
    ...
    def _create_approval_pipeline(self, name="Approval Pipeline"):
        ...
        st = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pl, name=f"{name} Stage",
            display_order=1, requires_quotation=True,
...
class LeadSourcePipelineTests(CRMBaseTestCase):
    ...
    def test_create_lead_source(self):
        response = self.client.post("/api/crm/lead-sources/", {
            "name": "Trade Show", "description": "Annual tech expo",
        }, format="json")
    ...
    def test_create_pipeline(self):
        response = self.client.post("/api/crm/pipelines/", {
            "name": "API Pipeline", "description": "Created via API",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_employee_cannot_manage_lead_sources(self):
        ...
        response = emp_client.post("/api/crm/lead-sources/", {
            "name": "Emp Source",
        }, format="json")
    ...
    def test_employee_cannot_manage_pipelines(self):
        emp_client, _, _ = self._create_employee_client()
        response = emp_client.post("/api/crm/pipelines/", {
            "name": "Emp Pipeline",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_manager_can_manage_lead_sources(self):
        ...
        response = mgr_client.post("/api/crm/lead-sources/", {
            "name": "Manager Source",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_manager_can_manage_pipelines(self):
        mgr_client, _, _ = self._create_manager_client()
        response = mgr_client.post("/api/crm/pipelines/", {
...
class LeadTests(CRMBaseTestCase):
    ...
    def setUp(self):
        ...
        self.lead = CRMService.create_lead(
            user=self.user, name="Default Lead", email="defaultlead@example.com",
            phone="1000000001", source=self.source, assigned_to=self.user,
    ...
    def test_lead_update_creates_audit_log(self):
        response = self.client.patch(
            f"/api/crm/leads/{self.lead.id}/", {"name": "Updated Lead"},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        audit = AuditLog.objects.filter(
            entity_id=self.lead.id, action="LEAD_UPDATED",
        ).first()
    ...
    def test_create_lead_with_inactive_source_rejected(self):
        response = self.client.post("/api/crm/leads/", {
            "name": "Bad", "source": str(self.inactive_source.id),
            "assigned_to": str(self.user.user_id),
            "pipeline": str(self.pipeline.id),
            "current_stage": str(self.stage1.id),
        }, format="json")
    ...
    def test_create_lead_with_inactive_pipeline_rejected(self):
        response = self.client.post("/api/crm/leads/", {
            "name": "Bad", "source": str(self.source.id),
            "assigned_to": str(self.user.user_id),
            "pipeline": str(self.inactive_pipeline.id),
            "current_stage": str(self.stage1.id),
        }, format="json")
    ...
    def test_patch_converted_lead_rejected(self):
        CRMService.convert_lead(
            user=self.user, lead=self.lead, name="Conv Customer",
            email="defaultlead@example.com", phone="1000000001",
        )
    ...
    def test_patch_lost_reason_on_active_lead_rejected(self):
        response = self.client.patch(
            f"/api/crm/leads/{self.lead.id}/", {"lost_reason": "not lost"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_assign_lead(self):
        new_user = User.objects.create_user(
            username="new_assignee", email="newassignee@example.com",
            password="Password123!", role=self.role,
        )
        response = self.client.post(f"/api/crm/leads/{self.lead.id}/assign/", {
    ...
    def test_assign_lead_missing_field_rejected(self):
        response = self.client.post(
            f"/api/crm/leads/{self.lead.id}/assign/", {}, format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_mark_lead_lost(self):
        response = self.client.post(f"/api/crm/leads/{self.lead.id}/lost/", {
    ...
    def test_reengagement_creates_audit_log(self):
        ...
        log = AuditLog.objects.filter(
            entity_id=self.lead.id, action="LEAD_REENGAGED",
        ).first()
    ...
    def test_convert_active_lead_creates_customer(self):
        response = self.client.post(f"/api/crm/leads/{self.lead.id}/convert/", {
            "name": "Conv Customer", "email": "defaultlead@example.com",
            "phone": "1000000001", "company_name": "Conv Corp",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_convert_lead_uses_defaults_when_fields_omitted(self):
        response = self.client.post(
            f"/api/crm/leads/{self.lead.id}/convert/", {}, format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["name"], "Default Lead")
    ...
    def test_convert_lead_missing_email_rejected(self):
        no_email_lead = CRMService.create_lead(
            user=self.user, name="No Email", source=self.source,
    ...
    def test_convert_lead_missing_phone_rejected(self):
        no_phone_lead = CRMService.create_lead(
            user=self.user, name="No Phone", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
            phone=None,
        )
        response = self.client.post(f"/api/crm/leads/{no_phone_lead.id}/convert/", {
    ...
    def test_convert_already_converted_lead_rejected(self):
        CRMService.convert_lead(
            user=self.user, lead=self.lead, name="Already Conv",
            email="defaultlead@example.com", phone="1000000001",
        )
        response = self.client.post(f"/api/crm/leads/{self.lead.id}/convert/", {
    ...
    def test_convert_lead_duplicate_customer_email_rejected(self):
        CRMService.convert_lead(
            user=self.user, lead=self.lead, name="First Customer",
            email="shared@example.com", phone="1000000001",
        )
        lead2 = CRMService.create_lead(
            user=self.user, name="Lead 2", email="lead2@example.com",
            phone="2000000002", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        response = self.client.post(f"/api/crm/leads/{lead2.id}/convert/", {
            "email": "shared@example.com", "phone": "2000000002",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_convert_lost_lead_rejected(self):
        CRMService.mark_lead_lost(user=self.user, lead=self.lead, lost_reason="Budget")
        response = self.client.post(f"/api/crm/leads/{self.lead.id}/convert/", {
            "email": "x@x.com", "phone": "1234567890",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_convert_nonexistent_lead_returns_404(self):
        fake_uuid = "00000000-0000-0000-0000-000000000099"
        response = self.client.post(f"/api/crm/leads/{fake_uuid}/convert/", {
            "email": "x@x.com", "phone": "1234567890",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_convert_lead_quotation_required_but_missing_rejected(self):
        ...
        st = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pl, name="ReqQ Stage",
            display_order=1, requires_quotation=True,
        )
        lead = CRMService.create_lead(
            user=self.user, name="ReqQ Lead", email="reqq@example.com",
            phone="3000000003", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
        response = self.client.post(f"/api/crm/leads/{lead.id}/convert/", {
            "email": "reqq@example.com", "phone": "3000000003",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_convert_lead_quotation_accepted_succeeds(self):
        ...
        st = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pl, name="AccQ Stage",
            display_order=1, requires_quotation=True,
        )
        lead = CRMService.create_lead(
            user=self.user, name="AccQ Lead", email="accq@example.com",
            phone="4000000004", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
    ...
    def test_employee_can_create_lead(self):
        ...
        response = emp_client.post("/api/crm/leads/", {
            "name": "Emp Lead", "source": str(self.source.id),
    ...
    def test_employee_cannot_assign_lead(self):
        emp_client, emp_user, _ = self._create_employee_client()
        lead = CRMService.create_lead(
            user=emp_user, name="Emp Lead", source=self.source,
            assigned_to=emp_user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        response = emp_client.post(f"/api/crm/leads/{lead.id}/assign/", {
            "assigned_to": str(emp_user.user_id),
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_employee_cannot_progress_lead(self):
        ...
        response = emp_client.post(f"/api/crm/leads/{lead.id}/progress/", {
            "stage_id": str(self.stage2.id),
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_employee_cannot_mark_lead_lost(self):
        ...
        response = emp_client.post(f"/api/crm/leads/{lead.id}/lost/", {
            "lost_reason": "Budget",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_employee_cannot_convert_lead(self):
        ...
        response = emp_client.post(f"/api/crm/leads/{lead.id}/convert/", {
...
class CustomerTests(CRMBaseTestCase):
    ...
    def _make_customer(self, name="Cust", email="c@example.com", phone="111"):
        lead = CRMService.create_lead(
            user=self.user, name=f"{name} Lead", email=email,
            phone=phone, source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        return CRMService.convert_lead(
            user=self.user, lead=lead, name=name,
    ...
    def test_customer_create_via_direct_post(self):
        lead = CRMService.create_lead(
            user=self.user, name="Post Lead", email="post@x.com",
            phone="444", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        response = self.client.post("/api/crm/customers/", {
            "lead": str(lead.id), "name": "Post Customer",
            "email": "post@x.com", "phone": "444",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_customer_update_returns_forbidden(self):
        ...
        response = self.client.patch(
            f"/api/crm/customers/{cust.id}/", {"name": "Nope"},
            format="json",
        )
        self.assertIn(response.status_code, [
    ...
    def test_duplicate_email_rejected_via_lead_conversion(self):
        ...
        lead2 = CRMService.create_lead(
            user=self.user, name="Lead 2", email="lead2@x.com",
            phone="666", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        response = self.client.post(f"/api/crm/leads/{lead2.id}/convert/", {
            "email": "dup@x.com", "phone": "666",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_customer_create_triggers_audit_log(self):
        lead = CRMService.create_lead(
            user=self.user, name="Audit Lead", email="audit@x.com",
            phone="777", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        response = self.client.post("/api/crm/customers/", {
            "lead": str(lead.id), "name": "Audit Cust",
            "email": "audit@x.com", "phone": "777",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_customer_activities_returns_activities(self):
        ...
        CRMService.create_activity(
            user=self.user, activity_type="CALL",
            outcome="Discussed requirements", customer=cust,
        )
        response = self.client.get(f"/api/crm/customers/{cust.id}/activities/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class ActivityTests(CRMBaseTestCase):
    ...
    def test_create_activity_for_lead(self):
        lead = CRMService.create_lead(
            user=self.user, name="Act Lead", email="actlead@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_create_activity_for_customer(self):
        lead = CRMService.create_lead(
            user=self.user, name="Act Cust Lead", email="actcust@x.com",
            phone="2222222222", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        customer = CRMService.convert_lead(
            user=self.user, lead=lead, name="Act Cust",
            email="actcust@x.com", phone="2222222222",
        )
        response = self.client.post("/api/crm/activities/", {
            "customer": str(customer.id), "activity_type": "MEETING",
    ...
    def test_create_activity_without_lead_or_customer_rejected(self):
        response = self.client.post("/api/crm/activities/", {
            "activity_type": "EMAIL", "outcome": "Sent follow-up",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_create_activity_with_both_lead_and_customer_rejected(self):
        lead = CRMService.create_lead(
            user=self.user, name="Both Lead", email="both@x.com",
            phone="3333333333", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        customer = CRMService.convert_lead(
            user=self.user, lead=lead, name="Both Cust",
            email="both@x.com", phone="3333333333",
        )
        response = self.client.post("/api/crm/activities/", {
            "lead": str(lead.id), "customer": str(customer.id),
            "activity_type": "CALL", "outcome": "Conflicting",
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_follow_up_requires_date(self):
        lead = CRMService.create_lead(
            user=self.user, name="FU Lead", email="fu@x.com",
            phone="4444444444", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        response = self.client.post("/api/crm/activities/", {
            "lead": str(lead.id), "activity_type": "FOLLOW_UP",
            "outcome": "Schedule next call", "follow_up_required": True,
        }, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_follow_up_date_without_flag_rejected(self):
        lead = CRMService.create_lead(
            user=self.user, name="Flag Lead", email="flag@x.com",
            phone="5555555555", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        response = self.client.post("/api/crm/activities/", {
            "lead": str(lead.id), "activity_type": "EMAIL",
    ...
    def test_create_activity_for_converted_lead_rejected(self):
        lead = CRMService.create_lead(
            user=self.user, name="Conv Lead", email="conv@x.com",
            phone="6666666666", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        CRMService.convert_lead(
            user=self.user, lead=lead, name="Conv Cust",
            email="conv@x.com", phone="6666666666",
        )
        response = self.client.post("/api/crm/activities/", {
            "lead": str(lead.id), "activity_type": "CALL",
            "outcome": "Should fail",
    ...
    def test_list_activities(self):
        lead = CRMService.create_lead(
            user=self.user, name="List Lead", email="list@x.com",
            phone="7777777777", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        CRMService.create_activity(
            user=self.user, activity_type="DEMO",
            outcome="Demo completed", lead=lead,
        )
    ...
    def test_activity_creation_creates_audit_log(self):
        lead = CRMService.create_lead(
            user=self.user, name="Audit Act Lead", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        activity = CRMService.create_activity(
            user=self.user, activity_type="CALL",
            outcome="Discussed pricing", lead=lead,
        )
        log = AuditLog.objects.filter(
...
class QuotationCRUDTests(CRMBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.lead = CRMService.create_lead(
            user=self.user, name="QC Lead", email="qclead@example.com",
            phone="2828282828", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_create_retrieve_update_quotation(self):
        response = self.client.post("/api/crm/quotations/", {
            ...
            "terms": "Net 30", "notes": "Initial draft",
            "line_items": [
        ...
        self.assertAlmostEqual(
            ...
            Decimal("200.00"), places=2,
        )

        resp = self.client.patch(f"/api/crm/quotations/{qid}/update-draft/", {
            "terms": "Net 15",
            "line_items": [{"description": "Product A", "quantity": 3, "unit_price": "50.00"}],
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertAlmostEqual(
            Decimal(resp.data["current_version_detail"]["total_amount"]),
            Decimal("150.00"), places=2,
        )

    def test_quotation_number_is_unique(self):
        q1 = QuotationService.create_quotation(
            user=self.user, lead=self.lead,
    ...
    def test_update_draft_replaces_all_line_items(self):
        ...
        resp = self.client.patch(f"/api/crm/quotations/{q.id}/update-draft/", {
            "line_items": [{"description": "Only", "quantity": 5, "unit_price": 20}],
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        q.refresh_from_db()
    ...
    def test_create_quotation_with_empty_line_items(self):
        q = QuotationService.create_quotation(
            user=self.user, lead=self.lead, line_items=[],
        )
        q.refresh_from_db()
    ...
    def test_create_quotation_for_converted_lead_rejected(self):
        CRMService.convert_lead(
            user=self.user, lead=self.lead, name="Conv Cust",
            email="qclead@example.com", phone="2828282828",
        )
    ...
    def test_revision_from_draft_creates_new_version(self):
        ...
        resp = self.client.post(f"/api/crm/quotations/{q.id}/revision/", {
    ...
    def test_reject_approval_returns_to_draft(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="RA Lead", source=self.source,
        ...
        resp = self.client.post(f"/api/crm/quotations/{q.id}/reject-approval/", {
    ...
    def test_quotation_list_filter_by_lead(self):
        lead2 = CRMService.create_lead(
            user=self.user, name="Filter Lead", email="filter@x.com",
            phone="9999999999", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        QuotationService.create_quotation(
            user=self.user, lead=self.lead,
    ...
    def test_cannot_accept_pending_approval_quotation(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Pending Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
    ...
    def test_cannot_send_pending_approval_quotation(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Pending2 Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
    ...
    def test_cannot_resubmit_approved_quotation(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Resubmit Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
    ...
    def test_full_invalid_transition_matrix(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="SM Lead", email="sm@x.com",
            phone="5552223333", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
        ...
        for action in ["accept", "reject", "send"]:
            self.assertEqual(
                self.client.post(f"/api/crm/quotations/{q.id}/{action}/",
        ...
        QuotationService.approve_quotation(
            reviewer_user=mgr_user, quotation=q,
        )
        q.refresh_from_db()
...
class QuotationWorkflowTests(CRMBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.lead = CRMService.create_lead(
            user=self.user, name="QW Lead", email="qwlead@example.com",
            phone="9876543210", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_submit_approve_send_accept_full_lifecycle(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="LC Lead", email="lclead@example.com",
            phone="1234567890", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
    ...
    def test_approval_workflow_requires_manager(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Appr Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
    ...
    def test_reject_quotation_marks_lead_lost(self):
        ...
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
        q = QuotationService.send_quotation(user=self.user, quotation=q)
        resp = self.client.post(f"/api/crm/quotations/{q.id}/reject/", {
    ...
    def test_reject_already_rejected_quotation_rejected(self):
        ...
        QuotationService.reject_quotation(
            user=self.user, quotation=q, rejection_reason="Price",
        )
        q.refresh_from_db()
        resp = self.client.post(f"/api/crm/quotations/{q.id}/reject/", {
    ...
    def test_duplicate_send_prevented(self):
        ...
        event_count = QuotationIntegrationEvent.objects.filter(
            quotation=q, event_type="quotation.followup_required",
        ).count()
    ...
    def test_quotation_update_draft_creates_activity(self):
        ...
        self.client.patch(f"/api/crm/quotations/{q.id}/update-draft/", {
            "terms": "Updated terms",
        }, format="json")
        act = Activity.objects.filter(
            quotation=q, activity_type="QUOTATION_UPDATED",
        ).first()
    ...
    def test_pipeline_stage_enforces_quotation_gate(self):
        ...
        st = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pl, name="Gate Stage",
            display_order=1, requires_quotation=True,
        )
        lead = CRMService.create_lead(
            user=self.user, name="Gate Lead", email="gate@x.com",
            phone="1112223333", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
        with self.assertRaises(ValidationError):
            CRMService.convert_lead(
                user=self.user, lead=lead, name="Gate Cust",
                email="gate@x.com", phone="1112223333",
            )
...
class QuotationVersioningTests(CRMBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.lead = CRMService.create_lead(
            user=self.user, name="Rev Lead", email="rev@example.com",
            phone="5553334444", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_v1_historical_immutability_after_revision(self):
        q = QuotationService.create_quotation(
            user=self.user, lead=self.lead, terms="Terms v1", notes="Notes v1",
            line_items=[{"description": "v1", "quantity": 2, "unit_price": 500}],
        )
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
        QuotationService.send_quotation_email(
            user=self.user, quotation=q, recipient_email="rev@example.com",
        )
        q.refresh_from_db()
    ...
    def test_multi_version_v1_v2_v3_accept_specific_version(self):
        q = QuotationService.create_quotation(
            user=self.user, lead=self.lead, terms="v1",
            line_items=[{"description": "v1", "quantity": 2, "unit_price": 500}],
        )
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
        QuotationService.send_quotation(user=self.user, quotation=q)
        ...
        # v2
        resp = self.client.post(f"/api/crm/quotations/{q.id}/revision/", {
            "revision_reason": "Discount", "terms": "v2",
            "line_items": [{"description": "v2", "quantity": 2, "unit_price": 400}],
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        q.refresh_from_db()
        ...
        # v3
        resp = self.client.post(f"/api/crm/quotations/{q.id}/revision/", {
            "revision_reason": "Add service pack", "terms": "v3",
            "line_items": [
    ...
    def test_revision_reason_recorded(self):
        ...
        resp = self.client.post(f"/api/crm/quotations/{q.id}/revision/", {
            ...
            "line_items": [{"description": "Item", "quantity": 1, "unit_price": 90}],
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        q.refresh_from_db()
        self.assertEqual(q.current_version.revision_reason, "Customer feedback on pricing")
...
class QuotationPDFAndEmailTests(CRMBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.lead = CRMService.create_lead(
            user=self.user, name="PDF Lead", email="pdf@example.com",
            phone="5551112222", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_pdf_blocked_for_pending_approval(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="PDF Pending Lead", email="pdfpending@x.com",
            phone="5551112222", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
    ...
    def test_pdf_allowed_for_approved_quotation(self):
        ...
        st = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pl, name="PDF Stage",
            display_order=1, requires_quotation=True,
            quotation_approval_required=True,
        )
        lead = CRMService.create_lead(
            user=self.user, name="PDF Appr Lead", email="pdfappr@x.com",
            phone="5551112222", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead, terms="Net 30", notes="PDF Test",
            line_items=[
        ...
        mgr_user = User.objects.create_user(
            username="pdf_mgr", email="pdfmgr@x.com",
            password="Password123!", role=self.role,
        )
        ...
        audit = AuditLog.objects.filter(
            entity_id=q.id, action="QUOTATION_PDF_GENERATED",
        ).first()
    ...
    def test_email_sends_with_pdf_attachment(self):
        ...
        st = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pl, name="Email Stage",
            display_order=1, requires_quotation=True,
        )
        lead = CRMService.create_lead(
            user=self.user, name="Email Lead", email="emaillead@x.com",
            phone="5553334444", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead, terms="Initial Terms",
        ...
        audit = AuditLog.objects.filter(
            entity_id=q.id, action="QUOTATION_EMAIL_SENT",
        ).first()
        self.assertIsNotNone(audit, "QUOTATION_EMAIL_SENT audit should exist")
        act = Activity.objects.filter(
            quotation=q, activity_type="QUOTATION_EMAIL_SENT",
        ).first()
...
class PermissionAndRBACTests(CRMBaseTestCase):
    ...
    def test_employee_cannot_approve_quotation(self):
        ...
        lead = CRMService.create_lead(
            user=self.employee, name="Emp Lead", source=self.source,
            assigned_to=self.employee, pipeline=pl, current_stage=st,
        )
    ...
    def test_employee_cannot_reject_quotation(self):
        lead = CRMService.create_lead(
            user=self.employee, name="Emp Lead", source=self.source,
            assigned_to=self.employee, pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
            user=self.employee, lead=lead,
            line_items=[{"description": "X", "quantity": 1, "unit_price": 100}],
        )
        QuotationService.submit_quotation_for_approval(user=self.employee, quotation=q)
        QuotationService.send_quotation(user=self.employee, quotation=q)
        resp = self.emp_client.post(f"/api/crm/quotations/{q.id}/reject/", {
    ...
    def test_manager_can_approve_quotation(self):
        ...
        lead = CRMService.create_lead(
            user=self.employee, name="Mgr Lead", source=self.source,
            assigned_to=self.employee, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.employee, lead=lead)
        QuotationService.submit_quotation_for_approval(user=self.employee, quotation=q)
    ...
    def test_manager_can_reject_quotation(self):
        ...
        QuotationService.submit_quotation_for_approval(user=self.employee, quotation=q)
        QuotationService.send_quotation(user=self.employee, quotation=q)
        resp = self.mgr_client.post(f"/api/crm/quotations/{q.id}/reject/", {
    ...
    def test_self_approval_prevented(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Self Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.user, lead=lead)
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
    ...
    def test_employee_submits_manager_approves_full_cycle(self):
        ...
        lead = CRMService.create_lead(
            user=self.employee, name="FC Lead", source=self.source,
            assigned_to=self.employee, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.employee, lead=lead,
    ...
    def test_dynamic_permission_revocation_blocks_approval(self):
        lead = CRMService.create_lead(
            user=self.employee, name="Dyn Lead", source=self.source,
            assigned_to=self.employee, pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
            user=self.employee, lead=lead,
    ...
    def test_rbac_case_employee_cannot_approve(self):
        ...
        resp = self.emp_client.post(
            f"/api/crm/quotations/00000000-0000-0000-0000-000000000099/approve/",
        )
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_rbac_case_manager_without_own_permission_cannot_self_approve(self):
        ...
        mgr_c, mgr_u, _ = self._create_manager_client(
            permissions=["view_quotation", "add_quotation", "submit_quotation", "approve_quotation"],
            username="mno",
        )
        pl, st = self._create_approval_pipeline("MgrNoOwn Pipeline")
        lead = CRMService.create_lead(
            user=mgr_u, name="MgrNoOwn Lead", source=self.source,
            assigned_to=mgr_u, pipeline=pl, current_stage=st,
        )
    ...
    def test_rbac_case_manager_with_own_permission_can_self_approve(self):
        ...
        mgr_c, mgr_u, _ = self._create_manager_client(
            permissions=[
                "view_quotation", "add_quotation", "submit_quotation",
                "approve_quotation", "approve_own_quotation",
            ],
            username="mgr_own",
        )
        pl, st = self._create_approval_pipeline("MgrOwn Pipeline")
        lead = CRMService.create_lead(
            user=mgr_u, name="MgrOwn Lead", source=self.source,
            assigned_to=mgr_u, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=mgr_u, lead=lead)
        QuotationService.submit_quotation_for_approval(user=mgr_u, quotation=q)
    ...
    def test_rbac_case_manager_approves_other_users_quotation(self):
        ...
        lead = CRMService.create_lead(
            user=self.employee, name="MgrOther Lead", source=self.source,
            assigned_to=self.employee, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.employee, lead=lead)
        QuotationService.submit_quotation_for_approval(user=self.employee, quotation=q)
    ...
    def test_rbac_case_admin_approves_quotaion(self):
        ...
        lead = CRMService.create_lead(
            user=self.employee, name="Admin Lead", source=self.source,
            assigned_to=self.employee, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.employee, lead=lead)
        QuotationService.submit_quotation_for_approval(user=self.employee, quotation=q)
    ...
    def test_superadmin_can_self_approve(self):
        ...
        su = User.objects.create_superuser(
            ...
            password="Password123!", phone_number="5555556666",
        )
        pl, st = self._create_approval_pipeline("SU Pipeline")
        lead = CRMService.create_lead(
            user=su, name="SU Lead", source=self.source,
            assigned_to=su, pipeline=pl, current_stage=st,
        )
    ...
    def test_approval_persistence_records_submitted_by_and_reviewed_by(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Persist Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.user, lead=lead)
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
        ...
        approval = QuotationApproval.objects.filter(
            version=q.current_version, decision="PENDING",
        ).first()
    ...
    def test_employee_full_quotation_permission_matrix(self):
        ...
        lead = CRMService.create_lead(
            user=self.employee, name="Matrix Lead", source=self.source,
            assigned_to=self.employee, pipeline=pl, current_stage=st,
        )
        resp = self.emp_client.post("/api/crm/quotations/", {
            "lead": str(lead.id), "terms": "Emp Terms",
        })
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        qid = resp.data["id"]
        ...
        self.assertEqual(
            self.emp_client.patch(
                f"/api/crm/quotations/{qid}/update-draft/", {"terms": "Updated"},
            ).status_code,
            status.HTTP_200_OK,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.emp_client.post(f"/api/crm/quotations/{qid}/reject-approval/").status_code,
            status.HTTP_403_FORBIDDEN,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.emp_client.post(
                f"/api/crm/quotations/{qid}/reject/", {"rejection_reason": "x"},
            ).status_code,
            status.HTTP_403_FORBIDDEN,
        )
        self.assertEqual(
...
class AuditLogTests(CRMBaseTestCase):
    ...
    def test_lead_source_creation_logged(self):
        source = CRMService.create_lead_source(
            user=self.user, name="Audit Source", description="For audit",
        )
        log = AuditLog.objects.filter(
            entity_type="LeadSource", entity_id=source.id,
    ...
    def test_pipeline_creation_logged(self):
        pipeline = CRMService.create_pipeline(
            user=self.user, name="Audit Pipeline", description="For audit",
        )
        log = AuditLog.objects.filter(
            entity_type="Pipeline", entity_id=pipeline.id,
    ...
    def test_pipeline_stage_creation_logged(self):
        stage = CRMService.create_pipeline_stage(
            user=self.user, pipeline=self.pipeline,
            name="Audit Stage", display_order=20,
        )
        log = AuditLog.objects.filter(
            entity_type="PipelineStage", entity_id=stage.id,
    ...
    def test_lead_assignment_logged(self):
        lead = CRMService.create_lead(
            user=self.user, name="Assign Lead", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        new_user = User.objects.create_user(
            username="assign_target", email="assign@x.com",
            password="Password123!", role=self.role,
        )
        CRMService.assign_lead(user=self.user, lead=lead, new_assignee=new_user)
        log = AuditLog.objects.filter(
            entity_type="Lead", entity_id=lead.id, action="LEAD_ASSIGNED",
        ).first()
        self.assertIsNotNone(log)
    ...
    def test_lead_stage_change_logged(self):
        lead = CRMService.create_lead(
            user=self.user, name="Stage Lead", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        CRMService.progress_lead(user=self.user, lead=lead, stage_id=self.stage2.id)
        log = AuditLog.objects.filter(
            entity_type="Lead", entity_id=lead.id, action="STAGE_CHANGED",
        ).first()
        self.assertIsNotNone(log)

    def test_lead_conversion_logged(self):
        lead = CRMService.create_lead(
            user=self.user, name="Conv Lead", email="convlog@x.com",
            phone="3232323232", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        CRMService.convert_lead(
            user=self.user, lead=lead, name="Conv Cust",
            email="convlog@x.com", phone="3232323232",
        )
        log = AuditLog.objects.filter(
            entity_type="Lead", entity_id=lead.id, action="LEAD_CONVERTED",
        ).first()
        self.assertIsNotNone(log)
    ...
    def test_quotation_full_lifecycle_logged(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Lifecycle Lead", email="lc@x.com",
            phone="3333333333", source=self.source, assigned_to=self.user,
            pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
        ...
        self.assertIsNotNone(AuditLog.objects.filter(
            entity_type="Quotation", entity_id=q.id,
            action="QUOTATION_CREATED",
        ).first(), "QUOTATION_CREATED audit should exist")
        ...
        ).first(), "QUOTATION_SUBMITTED audit should exist")

        mgr = User.objects.create_user(
            username="lc_mgr", email="lc_mgr@x.com",
            password="Password123!", role=self.role,
        )
        ...
        ).first(), "QUOTATION_APPROVED audit should exist")
        ...
        ).first(), "QUOTATION_SENT audit should exist")
        ...
        ).first(), "QUOTATION_ACCEPTED audit should exist")

    def test_quotation_rejection_logged(self):
        lead = CRMService.create_lead(
            user=self.user, name="Reject Lead", email="rej@x.com",
            phone="3434343434", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
    ...
    def test_quotation_revision_logged(self):
        lead = CRMService.create_lead(
            user=self.user, name="Rev Lead", email="revlog@x.com",
            phone="3535353535", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
        ...
        QuotationService.create_revision(
            user=self.user, quotation=q, revision_reason="Client feedback",
        )
        log = AuditLog.objects.filter(
            entity_type="Quotation", entity_id=q.id,
    ...
    def test_audit_logs_listed_via_api(self):
        CRMService.create_lead(
            user=self.user, name="List Lead", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
...
class EndToEndScenarioTests(CRMBaseTestCase):
    ...
    def test_startup_solo_salesperson_full_cycle(self):
        solo_client, solo, _ = self._create_manager_client(
            permissions=[
                "view_lead", "add_lead", "change_lead",
                "view_quotation", "add_quotation", "change_quotation",
                "submit_quotation", "approve_quotation", "approve_own_quotation",
                "send_quotation", "accept_quotation", "reject_quotation",
                "request_quotation_revision", "generate_quotation_pdf",
                "view_customer", "add_customer",
                "view_activity", "add_activity",
                "manage_lead_source", "manage_pipeline", "manage_pipeline_stage",
            ],
            username="solo_sales", rolename="SoloSales",
        )
        pipeline = CRMService.create_pipeline(user=solo, name="Startup Pipeline")
        stage = CRMService.create_pipeline_stage(
            user=solo, pipeline=pipeline, name="Demo Stage",
            display_order=1, requires_quotation=True,
            quotation_approval_required=True,
        )
        lead = CRMService.create_lead(
            user=solo, name="Startup Prospect", email="startup@x.com",
            phone="4100000001", source=self.source, assigned_to=solo,
    ...
    def test_enterprise_pipeline_with_revision_and_discount(self):
        pipeline = CRMService.create_pipeline(user=self.user, name="Enterprise Pipeline")
        stage1 = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pipeline, name="Qualification",
            display_order=1, requires_quotation=True,
            quotation_approval_required=True,
        )
        lead = CRMService.create_lead(
            user=self.user, name="Enterprise Client", email="enterprise@x.com",
            phone="4200000001", source=self.source, assigned_to=self.user,
            pipeline=pipeline, current_stage=stage1,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead, terms="Enterprise License",
            line_items=[
        ...
        mgr_user = User.objects.create_user(
            username="ent_mgr", email="ent_mgr@x.com",
            password="Password123!", phone_number="4200000002", role=self.role,
        )
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
        QuotationService.approve_quotation(reviewer_user=mgr_user, quotation=q)
        QuotationService.send_quotation(user=self.user, quotation=q)
        ...
        resp = self.client.post(f"/api/crm/quotations/{q.id}/revision/", {
            ...
            "line_items": [
                {"description": "Enterprise Suite", "quantity": 1, "unit_price": 8500},
                {"description": "Implementation", "quantity": 1, "unit_price": 5000},
                {"description": "Training", "quantity": 3, "unit_price": 1000},
            ],
        }, format="json")
    ...
    def test_lost_lead_reengagement_then_conversion(self):
        lead = CRMService.create_lead(
            user=self.user, name="Reengage Prospect", email="reengage@x.com",
            phone="4300000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        CRMService.mark_lead_lost(
            user=self.user, lead=lead, lost_reason="Budget constraints",
        )
        lead.refresh_from_db()
        ...
        CRMService.convert_lead(
            user=self.user, lead=lead, name="Reengage Customer",
            email="reengage@x.com", phone="4300000001",
        )
        lead.refresh_from_db()
        self.assertEqual(lead.status, Lead.Status.CONVERTED)
    ...
    def test_quotation_email_with_custom_subject_and_body(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Email Lead", email="emaillead@x.com",
            phone="4400000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
    ...
    def test_pdf_for_specific_version_in_multi_version(self):
        lead = CRMService.create_lead(
            user=self.user, name="PDF Ver Lead", email="pdfver@x.com",
            phone="4500000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
        ...
        QuotationService.create_revision(
            user=self.user, quotation=q, revision_reason="Updated pricing",
            line_items=[{"description": "V2", "quantity": 2, "unit_price": 150}],
        )
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
        resp = self.client.get(f"/api/crm/quotations/{q.id}/pdf/?version=1")
    ...
    def test_multiple_quotations_for_same_lead(self):
        lead = CRMService.create_lead(
            user=self.user, name="Multi Lead", email="multi@x.com",
            phone="4600000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q1 = QuotationService.create_quotation(
            user=self.user, lead=lead,
            line_items=[{"description": "Proposal A", "quantity": 1, "unit_price": 500}],
        )
        q2 = QuotationService.create_quotation(
            user=self.user, lead=lead,
            line_items=[{"description": "Proposal B", "quantity": 1, "unit_price": 800}],
        )
    ...
    def test_full_workflow_with_email_and_acceptance(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Full WF Lead", email="fullwf@x.com",
            phone="4900000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead, terms="Net 30", notes="Standard terms",
            line_items=[
...
class PerformanceTests(CRMBaseTestCase):
    ...
    def test_quotation_list_query_count_bounded(self):
        ...
        stage_perf = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pipeline_perf,
            name="Perf Stage", display_order=1,
        )
        for i in range(15):
            lead = CRMService.create_lead(
                user=self.user, name=f"Perf Lead {i}",
                email=f"perf{i}@x.com", phone=f"555000{i:04d}",
                source=self.source, assigned_to=self.user,
                pipeline=pipeline_perf, current_stage=stage_perf,
            )
            QuotationService.create_quotation(
                user=self.user, lead=lead,
                line_items=[
...
class ModelCleanAndStrTests(CRMBaseTestCase):
    ...
    def test_lead_clean_valid(self):
        lead = CRMService.create_lead(
            user=self.user, name="Clean Lead", email="clean@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        lead.full_clean()
    ...
    def test_lead_clean_wrong_stage_pipeline(self):
        lead = CRMService.create_lead(
            user=self.user, name="Mismatch Lead", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_lead_clean_lost_without_reason(self):
        lead = CRMService.create_lead(
            user=self.user, name="LostNoReason", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_lead_clean_lost_without_timestamp(self):
        lead = CRMService.create_lead(
            user=self.user, name="LostNoTS", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        lead.status = Lead.Status.LOST
    ...
    def test_lead_clean_active_with_lost_reason(self):
        lead = CRMService.create_lead(
            user=self.user, name="ActiveLostReason", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_lead_clean_active_with_lost_at(self):
        lead = CRMService.create_lead(
            user=self.user, name="ActiveLostAt", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        lead.lost_at = timezone.now()
        with self.assertRaises(ValidationError):
    ...
    def test_lead_clean_lost_with_both(self):
        lead = CRMService.create_lead(
            user=self.user, name="LostBoth", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        lead.status = Lead.Status.LOST
        lead.lost_reason = "Budget"
    ...
    def test_lead_str(self):
        lead = CRMService.create_lead(
            user=self.user, name="Str Lead", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_customer_str(self):
        lead = CRMService.create_lead(
            user=self.user, name="CustStr", email="custstr@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        cust = CRMService.convert_lead(
            user=self.user, lead=lead, name="CustStr Customer",
            email="custstr@x.com", phone="1111111111",
        )
    ...
    def test_activity_clean_both_lead_and_customer(self):
        lead = CRMService.create_lead(
            user=self.user, name="Both Lead", email="bothclean@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        cust = CRMService.convert_lead(
            user=self.user, lead=lead, name="Both Cust",
            email="bothclean@x.com", phone="1111111111",
        )
        act = Activity(
            lead=lead, customer=cust, created_by=self.user,
            activity_type="CALL", outcome="test",
        )
        with self.assertRaises(ValidationError):
            act.full_clean()
    ...
    def test_activity_clean_follow_up_required_no_date(self):
        lead = CRMService.create_lead(
            user=self.user, name="FU Lead", email="fuclean@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        act = Activity(
            lead=lead, created_by=self.user,
            activity_type="CALL", outcome="test",
    ...
    def test_activity_clean_follow_up_date_without_flag(self):
        lead = CRMService.create_lead(
            user=self.user, name="FUD Lead", email="fudclean@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        act = Activity(
    ...
    def test_activity_str(self):
        act = Activity(
            created_by=self.user, activity_type="CALL",
    ...
    def test_auditlog_str(self):
        log = AuditLog(
            user=self.user, entity_type="Lead",
            entity_id=uuid4(), action="LEAD_CREATED",
        )
        self.assertEqual(str(log), "LEAD_CREATED - Lead")

    def test_quotation_str(self):
        lead = CRMService.create_lead(
            user=self.user, name="QStr Lead", email="qstr@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
    ...
    def test_quotationversion_str(self):
        lead = CRMService.create_lead(
            user=self.user, name="QVStr Lead", email="qvstr@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
    ...
    def test_quotationlineitem_amount_and_str(self):
        from customer_management.models import QuotationLineItem
        lead = CRMService.create_lead(
            user=self.user, name="QLI Lead", email="qli@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
            line_items=[{"description": "Test Item", "quantity": 3, "unit_price": "25.00"}],
        )
    ...
    def test_quotationapproval_str(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="ApprStr Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.user, lead=lead)
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
    ...
    def test_quotationintegrationevent_str(self):
        lead = CRMService.create_lead(
            user=self.user, name="QIE Lead", email="qie@x.com",
            phone="1111111111", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        q = QuotationService.create_quotation(user=self.user, lead=lead)
...
class ServiceEdgeCaseTests(CRMBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.lead = CRMService.create_lead(
            user=self.user, name="Edge Lead", email="edge@x.com",
            phone="5550000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_create_quotation_with_json_string_line_items(self):
        ...
        q = QuotationService.create_quotation(
            user=self.user, lead=self.lead, line_items=items,
        )
        q.refresh_from_db()
        self.assertEqual(q.current_version.line_items.count(), 1)
        self.assertEqual(q.current_version.total_amount, Decimal("100.00"))
    ...
    def test_create_quotation_with_invalid_json_string_line_items(self):
        q = QuotationService.create_quotation(
            user=self.user, lead=self.lead, line_items="not json at all",
        )
        q.refresh_from_db()
        self.assertEqual(q.current_version.total_amount, Decimal("0.00"))
    ...
    def test_update_draft_with_json_string_line_items(self):
        ...
        q = QuotationService.update_draft_quotation(
            user=self.user, quotation=q, line_items=items,
        )
        q.refresh_from_db()
    ...
    def test_update_draft_with_invalid_json_string(self):
        ...
        q = QuotationService.update_draft_quotation(
            user=self.user, quotation=q, line_items="invalid json",
        )
        q.refresh_from_db()
        self.assertEqual(q.current_version.line_items.count(), 0)
    ...
    def test_update_draft_with_string_items_in_list(self):
        ...
        q = QuotationService.update_draft_quotation(
            user=self.user, quotation=q, line_items=["bad", 123],
        )
        q.refresh_from_db()
        self.assertEqual(q.current_version.line_items.count(), 0)
    ...
    def test_update_draft_only_terms(self):
        ...
        q = QuotationService.update_draft_quotation(
            user=self.user, quotation=q, terms="New terms only",
        )
        q.refresh_from_db()
    ...
    def test_update_draft_only_notes(self):
        ...
        q = QuotationService.update_draft_quotation(
            user=self.user, quotation=q, notes="Some notes",
        )
        q.refresh_from_db()
    ...
    def test_revision_with_invalid_json_string(self):
        ...
        QuotationService.send_quotation(user=self.user, quotation=q)
        q = QuotationService.create_revision(
            user=self.user, quotation=q, line_items="not json",
    ...
    def test_revision_without_line_items_copies_from_previous(self):
        ...
        QuotationService.send_quotation(user=self.user, quotation=q)
        q = QuotationService.create_revision(
            user=self.user, quotation=q, revision_reason="Copy",
        )
        q.refresh_from_db()
        self.assertEqual(q.current_version.version_number, 2)
        self.assertEqual(q.current_version.line_items.count(), 1)
    ...
    def test_revision_without_line_items_or_notes_copies_terms_and_notes(self):
        q = QuotationService.create_quotation(
            user=self.user, lead=self.lead,
            terms="Original Terms", notes="Original Notes",
            line_items=[{"description": "A", "quantity": 1, "unit_price": 100}],
        )
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
        QuotationService.send_quotation(user=self.user, quotation=q)
    ...
    def test_revision_no_version_rejected(self):
        q = Quotation.objects.create(
            ...
            lead=self.lead, created_by=self.user,
            status="DRAFT",
        )
        with self.assertRaises(ValidationError):
            QuotationService.create_revision(
    ...
    def test_create_quotation_no_active_stages_rejected(self):
        ...
        with self.assertRaises(ValidationError):
            CRMService.create_lead(
                user=self.user, name="Should fail", source=self.source,
                assigned_to=self.user, pipeline=pl, current_stage=self.stage1,
            )
    ...
    def test_create_lead_not_first_stage_rejected(self):
        with self.assertRaises(ValidationError):
            CRMService.create_lead(
                user=self.user, name="Not First", source=self.source,
    ...
    def test_assign_lead_same_user_no_change(self):
        lead = CRMService.create_lead(
            user=self.user, name="Same Assign", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        result = CRMService.assign_lead(
            user=self.user, lead=lead, new_assignee=self.user,
        )
    ...
    def test_progress_lead_no_next_stage_rejected(self):
        ...
        st = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pl, name="Only Stage", display_order=1,
        )
        lead = CRMService.create_lead(
            user=self.user, name="Last Stage", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        with self.assertRaises(ValidationError):
    ...
    def test_mark_lead_lost_empty_reason_rejected(self):
        with self.assertRaises(ValidationError):
            CRMService.mark_lead_lost(
                user=self.user, lead=self.lead, lost_reason="",
            )

    def test_mark_lead_lost_already_lost_rejected(self):
        CRMService.mark_lead_lost(
            user=self.user, lead=self.lead, lost_reason="Budget",
        )
        with self.assertRaises(ValidationError):
            CRMService.mark_lead_lost(
                user=self.user, lead=self.lead, lost_reason="Again",
            )
    ...
    def test_convert_lead_already_converted_rejected(self):
        CRMService.convert_lead(
            user=self.user, lead=self.lead, name="Conv",
            email="edge@x.com", phone="5550000001",
        )
        with self.assertRaises(ValidationError):
            CRMService.convert_lead(
                user=self.user, lead=self.lead, name="Conv2",
                email="edge@x.com", phone="5550000001",
            )
    ...
    def test_convert_lead_lost_rejected(self):
        ...
        with self.assertRaises(ValidationError):
            CRMService.convert_lead(
                user=self.user, lead=self.lead, name="X",
                email="edge@x.com", phone="5550000001",
            )

    def test_convert_lead_duplicate_email_rejected(self):
        CRMService.convert_lead(
            user=self.user, lead=self.lead, name="First",
            email="edge@x.com", phone="5550000001",
        )
        lead2 = CRMService.create_lead(
            user=self.user, name="Lead2", email="lead2@x.com",
            phone="5550000002", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        with self.assertRaises(ValidationError):
            CRMService.convert_lead(
                user=self.user, lead=lead2, name="Second",
                email="edge@x.com", phone="5550000002",
            )

    def test_convert_lead_with_accepted_quotation(self):
        pl, st = self._create_approval_pipeline("ConvQ Pipeline")
        lead = CRMService.create_lead(
            user=self.user, name="ConvQ Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead,
    ...
    def test_convert_lead_quotation_required_but_not_accepted_rejected(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="ConvQReq Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.user, lead=lead)
        with self.assertRaises(ValidationError):
            CRMService.convert_lead(
                user=self.user, lead=lead, name="X",
                email="convqreq@x.com", phone="5550000003",
            )

    def test_create_activity_follow_up_date_without_flag(self):
        lead = CRMService.create_lead(
            user=self.user, name="FUActivity", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        with self.assertRaises(ValidationError):
            CRMService.create_activity(
                user=self.user, activity_type="CALL",
                outcome="Test", lead=lead,
                follow_up_date=timezone.now(),
            )

    def test_create_activity_follow_up_required_no_date(self):
        lead = CRMService.create_lead(
            user=self.user, name="FUReqActivity", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        with self.assertRaises(ValidationError):
            CRMService.create_activity(
    ...
    def test_create_activity_for_converted_lead_rejected(self):
        lead = CRMService.create_lead(
            user=self.user, name="ConvAct", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        CRMService.convert_lead(
            user=self.user, lead=lead, name="ConvActCust",
            email="convact@x.com", phone="5550000004",
        )
        with self.assertRaises(ValidationError):
            CRMService.create_activity(
                user=self.user, activity_type="CALL",
    ...
    def test_create_activity_both_lead_and_customer_rejected(self):
        lead = CRMService.create_lead(
            user=self.user, name="BothAct", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
        )
        cust = CRMService.convert_lead(
            user=self.user, lead=lead, name="BothActCust",
            email="bothact@x.com", phone="5550000005",
        )
        with self.assertRaises(ValidationError):
            CRMService.create_activity(
                user=self.user, activity_type="CALL",
                outcome="Test", lead=lead, customer=cust,
            )
    ...
    def test_reject_quotation_no_version_rejected(self):
        ...
        with self.assertRaises(ValidationError):
            QuotationService.reject_quotation(
                user=self.user, quotation=q, rejection_reason="X",
            )
    ...
    def test_reject_quotation_no_reason_rejected(self):
        ...
        with self.assertRaises(ValidationError):
            QuotationService.reject_quotation(
                user=self.user, quotation=q, rejection_reason="",
            )
    ...
    def test_reject_quotation_lost_lead_no_reject(self):
        CRMService.mark_lead_lost(
            user=self.user, lead=self.lead, lost_reason="Already lost",
        )
        pl2 = CRMService.create_pipeline(user=self.user, name="RejectLost Pipeline")
        st2 = CRMService.create_pipeline_stage(
            user=self.user, pipeline=pl2, name="Stage", display_order=1,
        )
        lead2 = CRMService.create_lead(
            user=self.user, name="RejectLost2", email="rl2@x.com",
            phone="5550000006", source=self.source, assigned_to=self.user,
            pipeline=pl2, current_stage=st2,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead2,
            line_items=[{"description": "X", "quantity": 1, "unit_price": 100}],
        ...
        QuotationService.send_quotation(user=self.user, quotation=q)
        CRMService.mark_lead_lost(
            user=self.user, lead=lead2, lost_reason="Other reason",
        )
        q.refresh_from_db()
        QuotationService.reject_quotation(
            user=self.user, quotation=q, rejection_reason="Too expensive",
...
class ViewsEdgeCaseTests(CRMBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.lead = CRMService.create_lead(
            user=self.user, name="VE Lead", email="velead@x.com",
            phone="5550000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_quotation_create_nonexistent_lead(self):
        fake = "00000000-0000-0000-0000-000000000099"
        resp = self.client.post("/api/crm/quotations/", {
            "lead_id": fake,
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_lead_progress_validation_error(self):
        CRMService.convert_lead(
            user=self.user, lead=self.lead, name="Conv",
            email="velead@x.com", phone="5550000001",
        )
    ...
    def test_lead_lost_validation_error(self):
        ...
        resp = self.client.post(f"/api/crm/leads/{self.lead.id}/lost/", {
            "lost_reason": "X",
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_lead_convert_validation_error(self):
        ...
        resp = self.client.post(f"/api/crm/leads/{self.lead.id}/convert/", {
            "email": "velead@x.com", "phone": "5550000001",
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_activity_create_validation_error(self):
        resp = self.client.post("/api/crm/activities/", {
            "activity_type": "CALL", "outcome": "Test",
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_lead_source_create_validation_error(self):
        ...
        resp = self.client.post("/api/crm/lead-sources/", {
            "name": "Dup Source",
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_pipeline_create_validation_error(self):
        ...
        resp = self.client.post("/api/crm/pipelines/", {
            "name": "Dup Pipeline",
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_pipeline_stage_create_validation_error(self):
        resp = self.client.post("/api/crm/pipeline-stages/", {
            "pipeline": str(self.inactive_pipeline.id),
            "name": "Fail Stage", "display_order": 1,
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_email_no_recipient(self):
        lead_no_email = CRMService.create_lead(
            user=self.user, name="No Email Lead", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
            email=None,
        )
    ...
    def test_approve_own_quotation_via_api_returns_403(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="Own Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.user, lead=lead)
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
...
class PDFUtilsTests(CRMBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.lead = CRMService.create_lead(
            user=self.user, name="PDF Utils Lead", email="pdfutils@x.com",
            phone="5550000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_pdf_with_customer(self):
        ...
        q2 = Quotation.objects.create(
            ...
            lead=self.lead, created_by=self.user,
            customer=q.customer, status="APPROVED",
        )
        v2 = QuotationVersion.objects.create(
            quotation=q2, version_number=1, status="APPROVED",
            created_by=self.user, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
        QuotationLineItem.objects.create(
            version=v2, description="Item", quantity=1, unit_price=100,
        )
    ...
    def test_pdf_lead_with_no_email_phone(self):
        lead_no_ep = CRMService.create_lead(
            user=self.user, name="No EP Lead", source=self.source,
            assigned_to=self.user, pipeline=self.pipeline, current_stage=self.stage1,
            email=None, phone=None,
        )
        q = QuotationService.create_quotation(
            user=self.user, lead=lead_no_ep,
            line_items=[{"description": "Item", "quantity": 1, "unit_price": 100}],
        )
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
...
class SerializerValidationTests(CRMBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.lead = CRMService.create_lead(
            user=self.user, name="Ser Lead", email="ser@x.com",
            phone="5550000001", source=self.source, assigned_to=self.user,
            pipeline=self.pipeline, current_stage=self.stage1,
        )
    ...
    def test_lead_serializer_update_terminal_status_rejected(self):
        ...
        CRMService.convert_lead(
            user=self.user, lead=self.lead, name="Conv",
            email="ser@x.com", phone="5550000001",
        )
        self.lead.refresh_from_db()
        serializer = LeadSerializer(
            self.lead, data={"name": "Updated"}, partial=True,
        )
        self.assertFalse(serializer.is_valid())
    ...
    def test_lead_serializer_lost_reason_on_non_lost_lead(self):
        from customer_management.serializers import LeadSerializer
        serializer = LeadSerializer(
            self.lead, data={"lost_reason": "test"}, partial=True,
        )
        self.assertFalse(serializer.is_valid())
    ...
    def test_pipeline_stage_serializer_inactive_pipeline(self):
        ...
        serializer = PipelineStageSerializer(data={
            "pipeline": str(self.inactive_pipeline.id),
            "name": "Test",
            "display_order": 1,
        })
    ...
    def test_audit_log_serializer(self):
        ...
        log = CRMService.create_audit_log(
            user=self.user, entity_type="Test",
            entity_id=uuid4(), action="TEST_ACTION",
        )
    ...
    def test_quotation_approval_serializer(self):
        ...
        lead = CRMService.create_lead(
            user=self.user, name="ApprSer Lead", source=self.source,
            assigned_to=self.user, pipeline=pl, current_stage=st,
        )
        q = QuotationService.create_quotation(user=self.user, lead=lead)
        QuotationService.submit_quotation_for_approval(user=self.user, quotation=q)
