...
function RecordPaymentDialog({ open, onOpenChange, customer, dueAmount, onSuccess }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-[12px] border border-outline-variant bg-white p-6">
        <DialogHeader className="pb-3 border-b border-outline-variant"><DialogTitle className="text-[18px] font-semibold tracking-tight text-[#111214]">Record payment — {customer?.name}</DialogTitle></DialogHeader>
        ...
        <form onSubmit={handleSubmit((vals) => record.mutateAsync({ amount: vals.amount, payment_date: vals.payment_date || undefined, method: vals.method, reference: vals.reference || undefined, notes: vals.notes || undefined }).then(() => { reset(); onOpenChange(false); onSuccess?.(); }))} className="flex flex-col gap-3 pt-1">
          <FormField id="pay_amount" label={`Amount (max ₹${Number(dueAmount).toLocaleString("en-IN")})`} error={errors.amount?.message}><Input id="pay_amount" type="number" step="0.01" placeholder="e.g. 10000" className="h-9 border-input focus-visible:border-primary focus-visible:ring-1 focus-visible:ring-primary" {...register("amount")} /></FormField>
          <div className="grid grid-cols-2 gap-3">
            <FormField id="pay_date" label="Date" error={errors.payment_date?.message}><Input id="pay_date" type="date" className="h-9 border-input focus-visible:border-primary focus-visible:ring-1 focus-visible:ring-primary" {...register("payment_date")} /></FormField>
            <FormField id="pay_method" label="Method">
              <select id="pay_method" className="flex h-9 w-full rounded-md border border-input bg-white px-3 py-1 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" {...register("method")}>
          ...
          <FormField id="pay_ref" label="Reference (optional)"><Input id="pay_ref" placeholder="Txn ID / Cheque No." className="h-9 border-input focus-visible:border-primary focus-visible:ring-1 focus-visible:ring-primary" {...register("reference")} /></FormField>
          <FormField id="pay_notes" label="Notes (optional)"><Textarea id="pay_notes" rows={2} className="border-input focus-visible:border-primary focus-visible:ring-1 focus-visible:ring-primary" {...register("notes")} /></FormField>
          <DialogFooter className="pt-3 border-t border-outline-variant">
            <Button type="button" variant="outline" className="h-9 border-outline-variant text-[#111214]" onClick={() => onOpenChange(false)}>Cancel</Button>
            <Button type="submit" disabled={record.isPending} className="h-9 bg-secondary hover:bg-[#E0532A] text-white px-4">{record.isPending ? "Saving…" : "Record payment"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button className="h-9 bg-secondary hover:bg-[#E0532A] text-white px-4 text-sm font-medium" onClick={() => setPayOpen(true)}>Record payment</Button>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 rounded-[12px] border border-outline-variant bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
          <CardHeader className="pb-3 border-b border-outline-variant"><CardTitle className="text-[14px] font-semibold text-[#111214]">Profile</CardTitle><CardDescription className="text-xs text-[#6B7280]">Originating lead {customer.lead ? String(customer.lead).slice(0, 8) : "—"} • {customer.email} • {customer.phone}</CardDescription></CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2 p-6">
            ...
            <div className="flex flex-col gap-1 md:col-span-2">
              ...
              {customer.lead ? (
                <Link to={`/leads/${customer.lead}`} className="inline-flex w-fit items-center gap-2 text-sm text-primary hover:underline">
                  View lead <Badge variant="outline" className="font-mono text-[10px] border-outline-variant text-[#111214]">{String(customer.lead).slice(0, 8)}…</Badge>
                </Link>
              ) : <span className="text-sm text-[#6B7280]">—</span>}
            </div>
            {lookup?.account ? (
              <div className="md:col-span-2 rounded-[8px] border border-outline-variant bg-[#f9f9ff] p-3">
        ...
        <Card className="rounded-[12px] border border-outline-variant bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
          <CardHeader className="pb-3 border-b border-outline-variant"><CardTitle className="text-[14px] font-semibold text-[#111214]">Portfolio Overview</CardTitle><CardDescription className="text-xs text-[#6B7280]">Across all pipelines for this account/contact</CardDescription></CardHeader>
          <CardContent className="flex flex-col gap-3 p-6">
            {lookupQuery.isLoading ? <p className="text-sm text-[#6B7280]">Loading portfolio…</p> : lookup?.match_found ? (
              <>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="rounded-[8px] border border-outline-variant bg-white p-3"><div className="text-lg font-bold text-[#111214]">{portfolio?.total_engagements ?? recent.length}</div><div className="text-[11px] font-semibold uppercase tracking-[0.05em] text-[#6B7280]">Projects</div></div>
                  <div className="rounded-[8px] border border-outline-variant bg-white p-3"><div className="text-lg font-bold text-[#059669]">{portfolio?.active_engagements ?? 0}</div><div className="text-[11px] font-semibold uppercase tracking-[0.05em] text-[#6B7280]">Active</div></div>
                  <div className="rounded-[8px] border border-outline-variant bg-white p-3"><div className="text-lg font-bold text-primary">{portfolio?.completed_engagements ?? 0}</div><div className="text-[11px] font-semibold uppercase tracking-[0.05em] text-[#6B7280]">Completed</div></div>
                </div>
                <div className="rounded-[8px] border border-outline-variant bg-[#f9f9ff] p-3">
      ...
      <Card className="rounded-[12px] border border-outline-variant bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
        <CardHeader className="pb-3 border-b border-outline-variant"><CardTitle className="text-[14px] font-semibold text-[#111214]">Projects (all engagements for this account)</CardTitle><CardDescription className="text-xs text-[#6B7280]">Each lead is a project/engagement. Income tracked per engagement.</CardDescription></CardHeader>
          ...
          {recent.length === 0 ? <p className="py-6 text-center text-sm text-[#6B7280] px-6">Only this customer so far. Create more leads with the same company/GST to see them grouped here.</p> : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="h-[44px] border-b border-outline-variant text-[11px] font-semibold uppercase tracking-[0.05em] text-[#6B7280]">
                ...
                <tbody>
                  {recent.map((r) => (
                    <tr key={r.id} className="h-[44px] border-b border-outline-variant last:border-0 hover:bg-[#f9f9ff]">
                      <td className="px-4"><Link to={`/leads/${r.id}`} className="font-medium text-[#111214] hover:text-primary hover:underline">{r.title}</Link></td>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="rounded-[12px] border border-outline-variant bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
          <CardHeader className="pb-3 border-b border-outline-variant"><CardTitle className="text-[14px] font-semibold text-[#111214]">Quotations</CardTitle><CardDescription className="text-xs text-[#6B7280]">Linked to the originating lead (and account)</CardDescription></CardHeader>
            ...
            {quotationsQuery.isLoading ? <p className="text-sm text-[#6B7280]">Loading…</p> : quotations.length === 0 ? <p className="text-sm text-[#6B7280]">No quotations for this customer yet.</p> : (
              <div className="flex flex-col gap-2">
                {quotations.slice(0, 5).map((q) => (
                  <Link key={q.id} to={`/quotations/${q.id}`} className="flex items-center justify-between rounded-[8px] border border-outline-variant bg-white px-3 py-3 hover:border-[#2563EB]/30 hover:bg-[#f9f9ff]">
                    ...
                    <Badge variant="outline" className="border-outline-variant text-[11px] font-semibold text-[#111214]">{q.status}</Badge>
                  </Link>
                ))}
                {quotations.length > 5 ? <Link to={`/quotations?lead=${customer.lead}`} className="text-xs font-medium text-primary hover:underline">View all →</Link> : null}
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="rounded-[12px] border border-outline-variant bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
          <CardHeader className="pb-3 border-b border-outline-variant"><CardTitle className="text-[14px] font-semibold text-[#111214]">Income Summary</CardTitle><CardDescription className="text-xs text-[#6B7280]">From pipeline engagements • Manual payments update status</CardDescription></CardHeader>
          <CardContent className="flex flex-col gap-3 p-6">
            ...
            <Button size="sm" className="mt-1 h-9 bg-secondary hover:bg-[#E0532A] text-white" onClick={() => setPayOpen(true)}>Record payment</Button>
              ...
              <div className="mt-2 border-t border-outline-variant pt-3">
                ...
                <div className="mt-2 flex flex-col gap-1.5">
                  {paymentList.slice(0, 5).map((p) => (
                    <div key={p.id} className="flex items-center justify-between rounded-[8px] border border-outline-variant bg-[#f9f9ff] px-3 py-2 text-xs">
